...
#
# Exit behavior:
#   exit 0 = success or advisory warnings only (warnings emitted as
#            hookSpecificOutput additionalContext JSON — exit-0 stderr is invisible)
#   exit 2 = blocking error (build-breaking issues: invalid JSON) — stderr fed to Claude
#
# Input schema (PostToolUse for Write/Edit):
# { "tool_name": "Write", "tool_input": { "file_path": "data/foo.json", "content": "..." } }
...
# Only check files in data/ or assets/
if ! echo "$FILE_PATH" | grep -qE '(^|/)(data|assets)/'; then
    exit 0
fi
...
# Scoped to data/ paths only (assets/ has looser conventions)
...
# exit 2 + stderr is the only non-JSON channel Claude actually sees.
if [ -n "$ERRORS" ]; then
    echo -e "=== Asset Validation: ERRORS (Blocking) ===$ERRORS\n===========================================\nFix these errors before proceeding." >&2
    exit 2
fi

# Report warnings (advisory -- non-blocking) via PostToolUse
# hookSpecificOutput additionalContext JSON. Bare stderr at exit 0 is
# invisible to the model.
if [ -n "$WARNINGS" ]; then
    CONTEXT="$(printf "%b" "=== Asset Validation: Warnings ===$WARNINGS\n==================================\n(Warnings are advisory. Fix before final commit.)")"
    if command -v jq >/dev/null 2>&1; then
        printf '%s' "$CONTEXT" | jq -Rsa '{hookSpecificOutput: {hookEventName: "PostToolUse", additionalContext: .}}'
    else
