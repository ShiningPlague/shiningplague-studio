...
Examples:
```
/brainstorming space horror           → uses global mode
/brainstorming space horror --review full   → forces full mode this run
...
**Trigger**: After playtest reports are generated (`/qa-plan` playtest evidence), or after
...
**Trigger**: At milestone review (`/retrospective`), at mid-sprint retrospectives,
...
## Session Close Reflections (Non-Verdict)

These are NOT APPROVE/REJECT gates. They are structured reflection prompts fired
at session close for all 3 Tier 1 directors. They return a Reflection Record, not
...
### CLOSE-CD — Creative Director Session Reflection

**Trigger**: At session close, after work has shipped or a natural stopping point
is reached. Fired in parallel with CLOSE-TD and CLOSE-PR.

**Context to pass**:
- Workstream name + activity (e.g. "Combat × Design")
- Bulleted list of decisions made this session
- Game pillars (from GDD or game-pillars doc)
- MDA aesthetics target (if defined)

**Prompt**:
> "This session is closing. Workstream: [workstream] × [activity].
> Decisions made: [list].
>
> As Creative Director, provide your session reflection:
> 1. Do these decisions strengthen or weaken the game's vision and pillars?
> 2. Does this work help fulfill the EXPERIENCE we want players to have?
>    → If no player experience / MDA target is defined yet: FLAG THIS as a gap.
> 3. Highest-priority creative risk for next session?
> 4. Narrative/gameplay/tone concerns to carry forward?
> 5. Dependencies other departments need to know about?
>
> Format: Reflection record, 200 words max."

**Output format**: Reflection Record (no verdict line)

---

### CLOSE-TD — Technical Director Session Reflection

**Trigger**: At session close. Fired in parallel with CLOSE-CD and CLOSE-PR.

**Context to pass**:
- Workstream name
- Systems modified, new autoloads/addons, schema changes
- Architecture decisions made (ADR references)
- Known technical debt flagged during work

**Prompt**:
> "This session is closing. Workstream: [workstream].
> Technical scope shipped: [systems].
> Architecture decisions: [list].
>
> As Technical Director:
> 1. Is the code/architecture in sound shape for next session?
> 2. Performance, memory, or scalability concerns?
> 3. Technical blockers for dependent workstreams?
> 4. Any drift from architecture principles?
>
> Format: Reflection record, 200 words max."
...
### CLOSE-PR — Producer Session Reflection

**Trigger**: At session close. Fired in parallel with CLOSE-CD and CLOSE-TD.

**Context to pass**:
- Workstream name + activity
- Scope delivered (features / specs / content)
- Blockers that arose
- All workstream state files (for cross-cutting awareness)

**Prompt**:
> "This session is closing. Workstream: [workstream] × [activity].
> Scope delivered: [list]. Blockers: [list].
>
> As Producer:
> 1. Did we deliver what we planned?
> 2. Scope creep or unexpected cuts?
> 3. Are we OVERDOING things? Is there a faster, more efficient way?
> 4. Which workstream should be prioritized next?
> 5. Blockers or dependencies between workstreams?
> 6. One essential thing for the designer to think about as food-for-thought
>    — something strategic and relevant. A question, not an answer.
>
> Format: Reflection record, 200 words max."
