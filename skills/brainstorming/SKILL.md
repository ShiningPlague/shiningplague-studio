...
**It is not part of this bundle.** The companion server ships with the optional `obra/superpowers` plugin. If that plugin is not installed, do not offer it — say one line and stay in the terminal. See `.claude/skills/brainstorming/visual-companion.md` for the check and the terminal fallback.

**Offering the companion (only when the plugin is installed):** When you anticipate that upcoming questions will involve visual content (mockups, layouts, diagrams), offer it once for consent:
