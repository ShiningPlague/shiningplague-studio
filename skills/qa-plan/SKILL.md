...
*Smoke tests are verified by the developer via `/regression-suite`. Reference
...
- [ ] Smoke check passes (run `/regression-suite` before QA hand-off)
- [ ] No regressions introduced
- [ ] Code reviewed (via `/code-review` or documented peer review)
- [ ] Story file updated to `Status: Complete` (at `/dev-story` completion)
````
...
- Run `/regression-suite` after all stories are implemented to gate QA hand-off
- For Logic/Integration stories, create the test files at the listed paths
  before marking stories done — `/dev-story` verifies them at completion"
