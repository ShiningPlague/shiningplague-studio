...
If [C]: output "No day-one patch required. Proceed to `/gate-check`." Stop.
...
- **Targeted smoke check** — run `/regression-suite` scoped to affected systems
...
1. Draft player-facing patch notes inline (community-manager dispatch for tone if needed)
...
- **Player communication is part of the patch** — draft player-facing patch notes inline (community-manager dispatch for tone if needed); they are a required output, not optional
